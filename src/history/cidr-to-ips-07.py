...
def count_unique_ips_and_display_ranges_optimized(cidr_list=None, file_path=None):
    ...
    # Initialize GeoLite2 ASN database reader
    ...
    for entry in cidr_ranges:
        try:
            ...
            if entry.startswith("AS"):  # ASN format, e.g., "AS12345"
                ...
                # Query the ASN database to get associated prefixes
                response = reader.asn(entry)
                if response:
                    network_ranges = [prefix for prefix in response.networks]
                else:
