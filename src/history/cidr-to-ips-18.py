            ...
            for network in network_ranges:
                ...
                if not verbose:
                    print(f"Processed CIDR: {network} | IP Count: {total_ip_count} | "
                          f"First IP: {ipaddress.ip_address(int(network.network_address))} | "
                          f"Last IP: {ipaddress.ip_address(int(network.broadcast_address))}")
                if verbose:
...
def benchmark_implementations(cidr_list, file_path, asn_to_cidr, verbose=False):
    ...
    for i, (func, description) in implementations.items():
        ...
        time_taken, memory_used = func(cidr_list=cidr_list, file_path=file_path, asn_to_cidr=asn_to_cidr, verbose=False)
