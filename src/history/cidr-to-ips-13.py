...
class RangeSet:
    ...
    def add_range(self, start, end):
        ...
        for current_range in self.ranges:
            # Check if the new range overlaps or is contiguous with the current range
            if new_range[1] < current_range[0] - 1:
                # No overlap, and the new range is before the current range
                updated_ranges.append(current_range)
            elif new_range[0] > current_range[1] + 1:
                # No overlap, and the new range is after the current range
                updated_ranges.append(current_range)
            else:
                # Overlap or contiguous; merge the ranges
