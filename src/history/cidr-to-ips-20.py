    ...
    for entry in cidr_ranges:
        try:
            ...
            for network in network_ranges:
                # Create a list of all IPs, including network and broadcast
                ips = list(network)
                first_ip = ips[0]
                last_ip = ips[-1]
                ip_count = len(ips)

                # Update the set of IPs (using string representation)
                ip_set.update(str(ip) for ip in ips)
    ...
    for entry in cidr_ranges:
        try:
            ...
            for network in network_ranges:
                ...
                # Update the set of IPs (using integer representation)
                ip_set.update(int(ip) for ip in ips)
                total_ip_count += ip_count
