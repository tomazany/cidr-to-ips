...
# Implementation 2: Using a set of integers (with requested change)
    ...
    for entry in cidr_ranges:
        try:
            ...
            for network in network_ranges:
                ...
                # Update the set of IPs using a range of integers
                ip_set.update(range(int(first_ip), int(last_ip) + 1))
                total_ip_count += ip_count
